...
- Create page for each wedding venue we want
    ...
    2. Create a new entry in `pages` called `venueXXXX` (e.g. `venueWasingPark`). Again I suggest you copy the `venueWasingPark` entry and update accordingly. The `venue` property must match the key you created in the `venues` object.
